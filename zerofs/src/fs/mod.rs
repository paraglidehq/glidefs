...
use crate::db::{Db, Open, SlateDbHandle, Txn};
...
pub struct ZeroFS {
    ...
    /// Optional lease coordinator for writer coordination during handoff
    lease_coordinator: Option<Arc<lease::LeaseCoordinator>>,
}
...
impl ZeroFS {
    ...
    /// Create a new typestate transaction.
    ///
    /// Returns a `Txn<Open>` that can be used for store operations.
    /// When done, call `save_counter()` then `txn.sequence().commit().await?`.
    pub fn begin_txn(&self) -> Result<Txn<Open>, FsError> {
        if self.db.is_read_only() {
            return Err(FsError::ReadOnlyFilesystem);
        }
        Ok(Txn::new(self.db.clone(), self.write_coordinator.clone()))
    }

    /// Set the lease coordinator for writer coordination during handoff.
    /// Must be called after the lease is acquired.
    pub fn set_lease_coordinator(&mut self, coordinator: Arc<lease::LeaseCoordinator>) {
        self.lease_coordinator = Some(coordinator);
    }
    ...
    /// Resolve a path string to its inode ID by walking the directory tree.
    ///
    /// # Arguments
    /// * `path` - Absolute path like "/.nbd/vm-1.raw" or "/foo/bar/file.txt"
    ///
    /// # Returns
    /// * `Ok(inode_id)` - The inode ID for the path
    /// * `Err(FsError::NotFound)` - If any component doesn't exist
    /// * `Err(FsError::NotDirectory)` - If a non-final component isn't a directory
    pub async fn resolve_path_to_inode(&self, path: &str) -> Result<InodeId, FsError> {
        const ROOT_INODE_ID: InodeId = 0;
        ...
        for (i, component) in components.iter().enumerate() {
            ...
            let child_inode = self
                .directory_store
                .get(current_inode, component.as_bytes())
                .await?;

            // If not the last component, verify it's a directory
            if !is_last {
                let inode = self.inode_store.get(child_inode).await?;
    ...
    pub async fn write(
        ...
        // Check if this inode is blocked during per-path lease handoff
        self.check_writes_allowed_for_inode(id)?;
    ...
    pub async fn create(
        ...
        self.check_writes_allowed_for_inode(dirid)?;
    ...
    pub async fn link(
        ...
        self.check_writes_allowed_for_inode(fileid)?;
        validate_filename(linkname)?;
    ...
    pub async fn remove(
        ...
        // Look up the file being removed before checking if it's blocked
        let (file_id, cookie) = self
            .directory_store
            .get_entry_with_cookie(dirid, name)
            .await?;

        // Check if the file inode is blocked during per-path lease handoff
        self.check_writes_allowed_for_inode(file_id)?;
    ...
    pub async fn rename(
        ...
        // Look up the source file before checking if it's blocked
        let (source_inode_id, source_cookie) = self
            .directory_store
            .get_entry_with_cookie(from_dirid, from_name)
            .await?;

        // Check if the source file inode is blocked during per-path lease handoff
        self.check_writes_allowed_for_inode(source_inode_id)?;
...
mod tests {
    ...
    async fn test_save_and_load_inode() {
        ...
        fs.inode_store.save_counter(&mut txn);
        txn.sequence().commit().await.unwrap();
    ...
    async fn test_read_only_mode_operations() {
        ...
        let mut txn = fs_rw.begin_txn().unwrap();
        fs_rw
            .inode_store
            .save(&mut txn, test_inode_id, &Inode::File(file_inode.clone()))
            .unwrap();
        fs_rw.inode_store.save_counter(&mut txn);
        txn.sequence().commit().await.unwrap();
        ...
        let result = fs_ro.begin_txn();
        assert!(
            result.is_err(),
            "begin_txn should fail in read-only mode"
        );
    }
    ...
    #[tokio::test]
    async fn test_per_inode_blocking_rejects_mutations() {
        use crate::fs::lease::{LeaseConfig, LeaseCoordinator};
        use slatedb::object_store::memory::InMemory;

        let mut fs = ZeroFS::new_in_memory().await.unwrap();

        // Create a file first so we have an inode to block
        let creds = test_creds();
        let auth: AuthContext = (&test_auth()).into();
        let attr = SetAttributes::default();

        let (file_id, _) = fs.create(&creds, 0, b"testfile.txt", &attr).await.unwrap();

        // Create a lease coordinator
        ...
        let coordinator = Arc::new(LeaseCoordinator::new(
            ...
            LeaseConfig {
                ...
                holder_name: "test-node".to_string(),
            },
        ));

        // Block only the specific file inode (simulating prepare_handoff_path)
        coordinator.blocked_inodes.insert(file_id);

        // Set the coordinator on the filesystem
        fs.set_lease_coordinator(Arc::clone(&coordinator));

        // Verify write to blocked inode is rejected
        let write_result = fs
            .write(&auth, file_id, 0, &bytes::Bytes::from("test data"))
            .await;
        assert!(
            matches!(write_result, Err(FsError::ReadOnlyFilesystem)),
            "write to blocked inode should fail with ReadOnlyFilesystem, got {:?}",
            write_result
        );

        // Verify setattr to blocked inode is rejected
        let setattr_result = fs.setattr(&creds, file_id, &attr).await;
        assert!(
            matches!(setattr_result, Err(FsError::ReadOnlyFilesystem)),
            "setattr to blocked inode should fail with ReadOnlyFilesystem, got {:?}",
            setattr_result
        );

        // Verify operations on OTHER inodes still succeed (root inode 0 is not blocked)
        let create_result = fs.create(&creds, 0, b"unblocked.txt", &attr).await;
        assert!(
            create_result.is_ok(),
            "create in unblocked directory should succeed, got {:?}",
            create_result
        );

        let mkdir_result = fs.mkdir(&creds, 0, b"unblocked_dir", &attr).await;
        assert!(
            mkdir_result.is_ok(),
            "mkdir in unblocked directory should succeed, got {:?}",
            mkdir_result
        );

        // Now unblock the inode and verify operations succeed
        coordinator.blocked_inodes.remove(&file_id);

        let write_result = fs
            .write(&auth, file_id, 0, &bytes::Bytes::from("now allowed"))
            .await;
        assert!(
            write_result.is_ok(),
            "write should succeed after unblocking, got {:?}",
            write_result
        );
    }

    #[tokio::test]
    async fn test_resolve_path_to_inode() {
        let fs = ZeroFS::new_in_memory().await.unwrap();
        let creds = test_creds();
        let attr = SetAttributes::default();

        // Root path should resolve to inode 0
        let root_inode = fs.resolve_path_to_inode("/").await.unwrap();
        assert_eq!(root_inode, 0);

        // Empty path should also resolve to root
        let empty_inode = fs.resolve_path_to_inode("").await.unwrap();
        assert_eq!(empty_inode, 0);

        // Create a directory structure: /foo/bar/file.txt
        let (foo_id, _) = fs.mkdir(&creds, 0, b"foo", &attr).await.unwrap();
        let (bar_id, _) = fs.mkdir(&creds, foo_id, b"bar", &attr).await.unwrap();
        let (file_id, _) = fs.create(&creds, bar_id, b"file.txt", &attr).await.unwrap();

        // Resolve various paths
        let resolved_foo = fs.resolve_path_to_inode("/foo").await.unwrap();
        assert_eq!(resolved_foo, foo_id);

        let resolved_bar = fs.resolve_path_to_inode("/foo/bar").await.unwrap();
        assert_eq!(resolved_bar, bar_id);

        let resolved_file = fs.resolve_path_to_inode("/foo/bar/file.txt").await.unwrap();
        assert_eq!(resolved_file, file_id);

        // Path with trailing slash should work
        let resolved_with_slash = fs.resolve_path_to_inode("/foo/bar/").await.unwrap();
        assert_eq!(resolved_with_slash, bar_id);

        // Non-existent path should error
        let not_found = fs.resolve_path_to_inode("/foo/nonexistent").await;
        assert!(matches!(not_found, Err(FsError::NotFound)));

        // Path through non-directory should error
        let not_dir = fs.resolve_path_to_inode("/foo/bar/file.txt/child").await;
        assert!(matches!(not_dir, Err(FsError::NotDirectory)));
    }
}
