...
use crate::fs::lease::{LeaseState, WriterLease};
...
impl From<LeaseState> for proto::LeaseState {
    fn from(state: LeaseState) -> Self {
        match state {
            LeaseState::Active => proto::LeaseState::Active,
            LeaseState::Releasing => proto::LeaseState::Releasing,
            LeaseState::Released => proto::LeaseState::Released,
        }
    }
}

impl From<WriterLease> for proto::WriterLease {
    fn from(lease: WriterLease) -> Self {
        proto::WriterLease {
            holder_id: lease.holder_id.to_string(),
            holder_name: lease.holder_name,
            acquired_at: lease.acquired_at,
            expires_at: lease.expires_at,
            state: proto::LeaseState::from(lease.state) as i32,
