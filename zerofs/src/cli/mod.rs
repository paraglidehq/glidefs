...
pub enum Commands {
    ...
    /// Writer lease management commands for coordinated handoff
    Lease {
        #[command(subcommand)]
        subcommand: LeaseCommands,
    },
}
...
pub enum LeaseCommands {
    /// Get current lease status for a specific path
    Status {
        #[arg(short, long)]
        config: PathBuf,
        /// Path to check lease status for (e.g., "/.nbd/vm-1.raw")
        #[arg(short, long)]
        path: String,
    },
    ...
    /// Prepare for writer handoff of a specific path (blocks writes, flushes data)
    PrepareHandoff {
        #[arg(short, long)]
        config: PathBuf,
    ...
    /// Complete handoff and release lease for a specific path
    CompleteHandoff {
        #[arg(short, long)]
        config: PathBuf,
