...
async fn start_rpc_servers(
    ...
    write_coordinator: Arc<crate::fs::write_coordinator::WriteCoordinator>,
    lease_coordinator: Option<Arc<crate::fs::lease::LeaseCoordinator>>,
    tracer: AccessTracer,
...
pub async fn run_server(
    ...
    let mut fs = init_result.fs;
    let checkpoint_params = init_result.checkpoint_params;

    // Set up lease coordinator if enabled (only for read-write mode)
    let lease_coordinator: Option<Arc<crate::fs::lease::LeaseCoordinator>> =
        if !db_mode.is_read_only() {
            if let Some(ref fs_config) = settings.filesystem {
                if let Some(ref lease_config) = fs_config.lease {
                    if lease_config.enabled {
                        info!("Lease coordination enabled, acquiring writer lease...");
                        let config = crate::fs::lease::LeaseConfig {
                            lease_duration_secs: lease_config.duration_secs(),
                            renewal_interval_secs: lease_config.renewal_interval_secs(),
                            holder_name: lease_config
                                .holder_name
                                .clone()
                                .unwrap_or_else(|| {
                        ...
                        let coordinator = Arc::new(crate::fs::lease::LeaseCoordinator::new(
                            init_result.object_store.clone(),
                            &init_result.db_path,
                            config,
                        ));
                        // Per-path lease coordination: leases are acquired per-path
                        // during prepare_handoff, not globally at startup
                        info!("Per-path lease coordination enabled");
                        Some(coordinator)
                    } else {
                        None
                    }
    ...
    // Wire up lease coordinator to ZeroFS for write blocking during handoff
    if let Some(ref coordinator) = lease_coordinator {
        if let Some(fs_mut) = Arc::get_mut(&mut fs) {
            fs_mut.set_lease_coordinator(Arc::clone(coordinator));
        } else {
            // This shouldn't happen since we haven't cloned fs yet
            warn!("Could not set lease coordinator: ZeroFS has multiple references");
        }
    }
    ...
    // Start lease renewal background task if lease coordination is enabled
    let lease_renewal_handle = lease_coordinator.as_ref().map(|coordinator| {
        coordinator.start_renewal_task(shutdown.clone())
    });
    ...
    let rpc_handles = start_rpc_servers(
        ...
        Arc::clone(&fs.write_coordinator),
    ...
    if let Some(lease_handle) = lease_renewal_handle {
    ...
    // Release all held leases on shutdown
    if let Some(ref coordinator) = lease_coordinator {
        info!("Releasing all held leases...");
        if let Err(e) = coordinator.release_all().await {
            tracing::error!("Failed to release leases: {}", e);
        } else {
            info!("All leases released");
        }
    }
